            ...
            if (Config.Path.ToString() != string.Empty)
            {
                try
                {
                    ...
                    if (Config.CreateMd5File == true)
                    {
                        ...
                        Array.Resize(ref modifiedFiles,j); // срезать массив под количесво модифицированных файлов
                        files = modifiedFiles; //потому что files исп. как источник данных для методов сжатия
                        LastFileComputed = files.Length; //фикс от переполнения в блоке if (LastFileComputed != files.Length)                                                                                                               
                        //---------------------------------------------------------------------------
                        //сжатие      
                        //---------------------------------------------------------------------------
